...
const TABLE_HEAD = ["Id", "Name", "Email", "Address", "Phone", "Delivery Method", "Products", ""];
...
axios.get("http://localhost:8080/api/v1/getAllOrders").then(response => {setOrders(response.data.orders) }).catch( err => console.log(err));
}, [])
  ...
  return (
    ...
    <Card className="h-full w-full overflow-scroll">
      <table className="w-full min-w-max table-auto text-left">
        ...
        <tbody>
          {orders?.map(({ id, firstName, lastName, address, city, country, email, phone, products, deliveryMeth, state}, index) => (
            <tr key={id} className="even:bg-blue-gray-50/50">
              ...
              <td className="p-4">
                  ...
                  {
                   JSON.parse(products).map((data) =>(console.log(data),
                    <Typography variant="small" color="blue-gray" className="font-formulafont-regular">
                     {data?.quantity} x {data?.team} {data?.type}
                      </Typography>
                  ))}
